...
function CreditCardButton({ children, isLoading, render, canProceed, ...props }: CreditCardPayButtonProps) {
  ...
  const handlePayment = async (e: Event | React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (canProceed && !canProceed()) {
      console.warn('Credit Card button was clicked, but validation failed.');
      return;
    }
  ...
  return (
    <button {...props} aria-disabled={disabled} disabled={disabled} id={id} ref={buttonRef} type="button">
      {children ?? 'Pay'}
    </button>
  );
}
CreditCardButton.displayName = 'CreditCardButton';
